...
Where $n_{B}$ is the magnetic spectral index, $A_{k}$ is the amplitude of the PMF power spectrum and k is the wavenumber. The two parameters of interest here are of course, $n_{B}$ and $A_{k}$.

The magnetic spectral index, in analogy with the scalar spectral index from the primordial power spectrum, gives us the scales over which the PMF is significant. For a scale invariant PMF we would expect to see $n_{B} = 0$. However, current constraints for $n_{B}$ place it much lower - somewhere between -3.0 and -1.5.
The value of $n_{B}$ is a means of determining the origin of PMFs with...

The power spectrum amplitude is used to determine the magnetic field strength smoothed over a range of 1 Mpc, $B_{1Mpc}$, with a scaling relation of:
\begin{displaymath}
A_{pmf} \propto B_{1 Mpc}\textsuperscript{4}
\end{displaymath}

to $B_{1Mpc}$, the magnetic field strength smoothed over a range of 1 Mpc. 
